      ...
      (cond ((eq? tag 'var)
            ...
            ((eq? tag 'lambda-opt)
             (let ((params (cadr e))
                   ...
                   (body (cadddr e)))
               (if (not (or (member v params) (eq? v opt-param)))
                   `(lambda-opt ,params ,(replace-gets-sets body v))
                   e)))
            ((eq? tag 'applic)
             (let ((f (cadr e))
                   (args (caddr e)))
      ...
      (cond ((eq? tag 'var)
            ...
            ((eq? tag 'lambda-opt)
             (let ((params (cadr e))
                   ...
                   (body (cadddr e)))
               (if (not (or (member v params) (eq? v opt-param)))
                   `(lambda-opt ,params ,(var->lex-var body v minor (+ depth 1)))
                   e)))
            ((eq? tag 'applic)
             (let ((f (cadr e))
                   (args (caddr e)))
